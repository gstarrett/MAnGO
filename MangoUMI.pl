...
my (%uniq,%uids,$insertSize,%fileHash,%uidsUMI);
...
for (my $n = 0; $n <= $#indexFiles; $n++) {
    ...
    if (exists $lengthHash{length($simpleRead)}) {
      ...
      my %UMIinternal = ($infile => $key);
			$uids{$newUID} = \%internal;
      $uidsUMI{$newUID} = \%UMIinternal;
      print FASTA ">" . $newUID . "\n$simpleRead\n"
		}
...
close(OUT);
open(UMI, "> $prefix.umi.txt");
for my $key (keys %uidsUMI) {
  for my $file (keys %{$uidsUMI{$key}}) {
    my @UMIs = split (",", ${$uidsUMI{$key}}{$file});
    for (@UMIs) {
      print UMI "$_\t$key\n";
    }
  }
